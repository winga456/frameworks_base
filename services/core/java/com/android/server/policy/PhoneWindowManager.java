...
import com.android.internal.util.vrtoxin.Converter;
import com.android.internal.util.vrtoxin.DexoptDialog;
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private void updateEdgeGestureListenerState() {
        ...
        if (mUsingEdgeGestureServiceForGestures) {
            ...
            if (mNavigationBar != null && !mNavigationBar.isVisibleLw() && !isStatusBarKeyguard() && !immersiveModeImplementsPaPie()) {
    ...
    public void updateSettings() {
        ...
        synchronized (mLock) {
            ...
            // Pa Pie
            mPaPieState = (Settings.System.getIntForUser(resolver,
            ...
            if (!mHasNavigationBar) {
                ...
                mNavigationBarHeightForRotation[mPortraitRotation] =
                mNavigationBarHeightForRotation[mUpsideDownRotation] = Converter.dpToPx(mContext,
                    Settings.System.getIntForUser(
                        mContext.getContentResolver(),
                        Settings.System.NAVIGATION_BAR_HEIGHT, 48,
                        UserHandle.USER_CURRENT));
                mNavigationBarHeightForRotation[mLandscapeRotation] =
                mNavigationBarHeightForRotation[mSeascapeRotation] = Converter.dpToPx(mContext,
                    Settings.System.getIntForUser(
                        mContext.getContentResolver(),
                ...
                mNavigationBarWidthForRotation[mUpsideDownRotation] =
                mNavigationBarWidthForRotation[mLandscapeRotation] =
                mNavigationBarWidthForRotation[mSeascapeRotation] =  Converter.dpToPx(mContext,
                    Settings.System.getIntForUser(
                        mContext.getContentResolver(),
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            if (scanCode != 0 && !mHasHwKeysEnabled) {
                Log.i(TAG, "Ignoring Home Key: we have hw keys turned off");
                return 0;
            }
            ...
            if (scanCode != 0 && !mHasHwKeysEnabled) {
                Log.i(TAG, "Ignoring Menu Key: we have hw keys turned off");
                return 0;
            }
            ...
            if (scanCode != 0 && !mHasHwKeysEnabled) {
                Log.i(TAG, "Ignoring Search Key: we have hw keys turned off");
                return 0;
            }
            ...
            if (scanCode != 0 && !mHasHwKeysEnabled) {
                Log.i(TAG, "Ignoring App Switch Key: we have hw keys turned off");
                return 0;
            }
            ...
            if (scanCode != 0 && !mHasHwKeysEnabled) {
                Log.i(TAG, "Ignoring Assist Key: we have hw keys turned off");
                return 0;
            }
            ...
            if (scanCode != 0 && !mHasHwKeysEnabled) {
                Log.i(TAG, "Ignoring Back Key: we have hw keys turned off");
                return 0;
            }
    ...
    private boolean immersiveModeImplementsPaPie() {
        return mPaPieState;
    }
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        final boolean down = event.getAction() == KeyEvent.ACTION_DOWN;
        final boolean canceled = event.isCanceled();
        final boolean longPress = (keyCode & KeyEvent.FLAG_LONG_PRESS) != 0;
        final boolean virtualKey = event.getDeviceId() == KeyCharacterMap.VIRTUAL_KEYBOARD;
    ...
    private void requestTransientBars(WindowState swipeTarget) {
        synchronized (mWindowManagerFuncs.getWindowManagerLock()) {
            ...
            if (sb || nb) {
                ...
                if (nb && !immersiveModeImplementsPaPie())
                    mNavigationBarController.showTransient();
