...
import com.android.internal.util.vrtoxin.PaPieConstants;
 ...
 */
public class PaPieMenu extends FrameLayout {
    ...
    private PaPieHelper mPieHelper;
    private Vibrator mVibrator;

    private PaPieItem mCurrentItem;
    private List<PaPieItem> mItems;
    private PaPieControlPanel mPanel;
    ...
    private void getDimensions() {
        ...
        mNowOnTap = Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.PA_PIE_NOW_ON_TAP, 0) == 1;
        ...
        // snap
        mSnapRadius = (int) (mResources.getDimensionPixelSize(R.dimen.pa_pie_snap_radius) * mPieSize);
        mSnapThickness = (int) (mResources
                .getDimensionPixelSize(R.dimen.pa_pie_snap_thickness) * mPieSize);
        ...
        mInnerPieRadius = (int) (mResources
                .getDimensionPixelSize(R.dimen.pa_pie_radius_start) * mPieSize);
        mOuterPieRadius = (int) (mInnerPieRadius +
                mResources.getDimensionPixelSize(R.dimen.pa_pie_radius_increment) * mPieSize);

        // calculate chevrons
        mInnerChevronRadius = (int) (mResources
                .getDimensionPixelSize(R.dimen.pa_pie_chevron_start) * mPieSize);
        mOuterChevronRadius = (int) (mInnerChevronRadius +
                mResources.getDimensionPixelSize(R.dimen.pa_pie_chevron_increment) * mPieSize);
        ...
        mStatusRadius = (int) (mResources
                .getDimensionPixelSize(R.dimen.pa_pie_status_start) * mPieSize);
        mStatusPath.reset();
        mStatusPath.addCircle(mCenter.x, mCenter.y, mStatusRadius, Path.Direction.CW);

        mClockPaint.setTextSize(mResources
                .getDimensionPixelSize(R.dimen.pa_pie_clock_size) * mPieSize);
        mClockOffset = mResources
                .getDimensionPixelSize(R.dimen.pa_pie_clock_offset) * mPieSize;
        mAmPmPaint.setTextSize(mResources
                .getDimensionPixelSize(R.dimen.pa_pie_am_pm_size) * mPieSize);
        mAmPmOffset = mResources.getDimensionPixelSize(R.dimen.pa_pie_am_pm_offset) * mPieSize;

        mStatusPaint.setTextSize((int) (mResources
                .getDimensionPixelSize(R.dimen.pa_pie_status_size) * mPieSize));
        mStatusOffset = mResources.getDimensionPixelSize(R.dimen.pa_pie_status_offset) * mPieSize;

        // battery
        mInnerBatteryRadius = (int) (mResources
                .getDimensionPixelSize(R.dimen.pa_pie_battery_start) * mPieSize);
        mOuterBatteryRadius = (int) (mInnerBatteryRadius +
                mResources.getDimensionPixelSize(R.dimen.pa_pie_battery_increment) * mPieSize);

        mBatteryBackground.setColor(getResources().getColor(R.color.battery_background));
        mBatteryLevel = mPieHelper.getBatteryLevel();
        if (mBatteryLevel <= PaPieHelper.LOW_BATTERY_LEVEL
                && mBatteryLevel > PaPieHelper.CRITICAL_BATTERY_LEVEL) {
        ...
        if (mEnableColor) {
            ...
            mBatteryJuice.setColorFilter(new PorterDuffColorFilter(extractRGB(Settings.System
                    ...
                    Mode.SRC_ATOP));
            for (PaPieItem item : mItems) {
                item.setColor(Settings.System.getInt(mContext.getContentResolver(),
                        Settings.System.PA_PIE_BUTTON_COLOR, COLOR_PIE_BUTTON));
            }
        } else {
            mPieBackground.setColor(getResources().getColor(R.color.pa_pie_background));
            mPieSelected.setColor(getResources().getColor(R.color.pa_pie_select));
            mPieOutlines.setColor(getResources().getColor(R.color.pa_pie_outlines));
     ...
     * @Param panel the current PaPieControlPanel
     */
    public PaPieMenu(Context context, PaPieControlPanel panel) {
        super(context);
        ...
        mPieHelper = PaPieHelper.getInstance();

        // initialize classes
        mItems = new ArrayList<PaPieItem>();
        ...
        mPieOutlines.setStrokeWidth(mResources.getDimensionPixelSize(R.dimen.pa_pie_outline));
        ...
        // Clock observer
        mPieHelper.setOnClockChangedListener(new PaPieHelper.OnClockChangedListener() {
    ...
    public void addItem(PaPieItem item) {
        mItems.add(item);
    }
    ...
    private boolean canItemDisplay(PaPieItem item) {
        return !(item.getName().equals(PaPieConstants.MENU_BUTTON) && !mPanel.currentAppUsesMenu()
                && !mUseMenuAlways) &&
                !(item.getName().equals(PaPieConstants.KILL_TASK_BUTTON) && !mUseKillTask) &&
                !(item.getName().equals(PaPieConstants.LAST_APP_BUTTON) && !mUseLastApp) &&
                !(item.getName().equals(PaPieConstants.NOW_ON_TAP) && !mNowOnTap) &&
                !(item.getName().equals(PaPieConstants.POWER_BUTTON) && !mUsePower) &&
                !(item.getName().equals(PaPieConstants.POWER_MENU_BUTTON) && !mUsePowerMenu) &&
                !(item.getName().equals(PaPieConstants.EXPANDED_DESKTOP_BUTTON) && !mExpandedDesktop) &&
                !(item.getName().equals(PaPieConstants.SCREENSHOT_BUTTON) && !mUseScreenshot);
    }
    ...
    private void drawItem(Canvas canvas, PaPieItem item) {
    ...
    public boolean onTouchEvent(MotionEvent evt) {
        ...
        if (MotionEvent.ACTION_DOWN == action) {
            ...
            if (mCenterDistance < shadeTreshold) {
                ...
                PaPieItem item = findItem(getPolar(mX, mY));
                if (item != null) {
                    if (mCenterDistance < shadeTreshold &&
    ...
    private void onEnter(PaPieItem item) {
    ...
    private PaPieItem findItem(float polar) {
        if (mItems != null) {
            for (PaPieItem item : mItems) {
                if (!canItemDisplay(item)) continue;
    ...
    private boolean inside(float polar, PaPieItem item) {
