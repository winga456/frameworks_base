...
public class FloatingButton extends QabButton {

    private final ContentObserver mFloatingObserver;
    private final ContentResolver mResolver;

    private boolean mEnabled;

    public FloatingButton(Context context, QuickAccessBar bar, Drawable iconEnabled,
            Drawable iconDisabled) {
        super(context, bar, iconEnabled, iconDisabled);

        mFloatingObserver = new ContentObserver(new Handler()) {
            @Override
    ...
    public void setListening(boolean listening) {
        if (listening) {
            mResolver.registerContentObserver(
                    ...
                    false, mFloatingObserver);
        } else {
            mResolver.unregisterContentObserver(mFloatingObserver);
        }
    }
    ...
    public void handleClick() {
        Settings.System.putInt(mResolver,
                Settings.System.FLOATING_WINDOW_MODE, mEnabled ? 0 : 1);
    }
