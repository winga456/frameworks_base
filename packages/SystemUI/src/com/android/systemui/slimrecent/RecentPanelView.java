...
public class RecentPanelView {
    ...
    private void handleFavoriteEntry(TaskDescription td) {
        ...
        final String favorites = Settings.System.getStringForUser(
                    resolver, Settings.System.SLIM_RECENTS_FAVORITES,
    ...
    private void constructMenu(final View selectedView, final TaskDescription td) {
        ...
        final Context layoutContext = new ContextThemeWrapper(mContext, getPopupThemeResource());
        ...
        if (Settings.System.getInt(mContext.getContentResolver(),
                Settings.System.DEVELOPMENT_SHORTCUT, 1) == 1) {
    ...
    private int getPopupThemeResource() {
        ContentResolver resolver = mContext.getContentResolver();

        int mode = Settings.System.getIntForUser(resolver,
                Settings.System.SLIM_RECENTS_POPUP_THEME_MODE, 1,
                UserHandle.USER_CURRENT);
        int themeDark = R.style.RecentBaseStyle;
        int themeLight = R.style.RecentBaseStyleLight;
        ...
        int defaultTheme = ColorHelper.isColorDark(cardColor) ?
                themeDark : themeLight;
        int themeResource = themeDark;

        if (mode == 0) {
            themeResource = defaultTheme;
        } else if (mode == 2) {
            themeResource = themeLight;
        }
    ...
    protected void loadTasks() {
        ...
        final String favorites = Settings.System.getStringForUser(
                mContext.getContentResolver(), Settings.System.SLIM_RECENTS_FAVORITES,
                UserHandle.USER_CURRENT);
        ...
        int maxNumTasksToLoad = Settings.System.getIntForUser(mContext.getContentResolver(),
                Settings.System.SLIM_RECENTS_MAX_APPS, ActivityManager.getMaxRecentTasksStatic(),
                UserHandle.USER_CURRENT);
