...
public class RecentController implements RecentPanelView.OnExitListener,
    ...
    private int getEmptyIconColor() {
        ContentResolver resolver = mContext.getContentResolver();
        Resources res = mContext.getResources();

        boolean useEmptyIconColor = Settings.System.getIntForUser(resolver,
                Settings.System.SLIM_RECENTS_USE_EMPTY_ICON_COLOR, 0,
                UserHandle.USER_CURRENT) == 1;
        int emptyIconColorLight = res.getColor(R.color.recents_icon_light_color);
        int emptyIconColorDark = res.getColor(R.color.recents_icon_dark_color);
        int defaultEmptyIconColor = ColorHelper.isColorDark(mPanelColor) ?
                emptyIconColorLight : emptyIconColorDark;
        int emptyIconColor = Settings.System.getIntForUser(resolver,
                Settings.System.SLIM_RECENTS_EMPTY_ICON_COLOR, defaultEmptyIconColor,
                UserHandle.USER_CURRENT);

        if (useEmptyIconColor) {
            return emptyIconColor;
        } else {
            return defaultEmptyIconColor;
        }
    }
    ...
    private class SettingsObserver extends ContentObserver {
        ...
        void observe() {
            ...
            resolver.registerContentObserver(Settings.System.getUriFor(
                    ...
                    false, this, UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.System.getUriFor(
                    Settings.System.SLIM_RECENTS_SCALE_FACTOR),
                    false, this, UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.System.getUriFor(
                    Settings.System.SLIM_RECENTS_EXPANDED_MODE),
                    false, this, UserHandle.USER_ALL);
            resolver.registerContentObserver(Settings.System.getUriFor(
        ...
        public void update() {
            ...
            mUserGravity = Settings.System.getIntForUser(
                    resolver, Settings.System.SLIM_RECENTS_GRAVITY, Gravity.RIGHT,
                    UserHandle.USER_CURRENT);

            // Get user panel color.
            int defaultBgColor = mContext.getResources().getColor(
                    R.color.panel_background);
            mPanelColor = Settings.System.getIntForUser(resolver,
                    Settings.System.SLIM_RECENTS_BG_COLOR, defaultBgColor,
                    UserHandle.USER_CURRENT);
            ...
            if (mRecentPanelView != null) {
                mRecentPanelView.setShowOnlyRunningTasks(Settings.System.getIntForUser(
                        resolver, Settings.System.SLIM_RECENTS_ONLY_SHOW_RUNNING_TASKS, 0,
                        UserHandle.USER_CURRENT) == 1);
                mRecentPanelView.setScaleFactor(mScaleFactor);
                mRecentPanelView.setExpandedMode(Settings.System.getIntForUser(
                        resolver, Settings.System.SLIM_RECENTS_EXPANDED_MODE,
