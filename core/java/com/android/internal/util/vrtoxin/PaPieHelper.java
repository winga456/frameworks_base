...
public class PaPieHelper {
    ...
    public static ArrayList<ActionConfig> getPaPieConfigWithDescription(
        ...
        if (config == null) {
            config = PaPieConstants.PA_PIE_CONFIG_DEFAULT;
        }
    ...
    public static Drawable getActionIconImage(Context context,
        ...
        if (customIcon != null && customIcon.startsWith(PaPieConstants.SYSTEM_ICON_IDENTIFIER)) {
            resId = systemUiResources.getIdentifier(customIcon.substring(
                        PaPieConstants.SYSTEM_ICON_IDENTIFIER.length()), "drawable", "android");
            if (resId > 0) {
                return systemUiResources.getDrawable(resId);
            }
        } else if (customIcon != null && !customIcon.equals(PaPieConstants.ICON_EMPTY)) {
    ...
    private static int getActionSystemIcon(Resources systemUiResources, String clickAction) {
        ...
        if (clickAction.equals(PaPieConstants.HOME_BUTTON)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_home", null, null);
        } else if (clickAction.equals(PaPieConstants.BACK_BUTTON)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_back", null, null);
        } else if (clickAction.equals(PaPieConstants.RECENT_BUTTON)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_recent", null, null);
        } else if (clickAction.equals(PaPieConstants.MENU_BUTTON)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_menu", null, null);
        } else if (clickAction.equals(PaPieConstants.MENU_BIG_BUTTON)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_menu_big", null, null);
        } else if (clickAction.equals(PaPieConstants.LAST_APP_BUTTON)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_lastapp", null, null);
        } else if (clickAction.equals(PaPieConstants.KILL_TASK_BUTTON)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_killtask", null, null);
        } else if (clickAction.equals(PaPieConstants.NOW_ON_TAP)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_now_on_tap", null, null);
        } else if (clickAction.equals(PaPieConstants.POWER_BUTTON)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_power", null, null);
        } else if (clickAction.equals(PaPieConstants.POWER_MENU_BUTTON)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_power_menu", null, null);
        } else if (clickAction.equals(PaPieConstants.EXPANDED_DESKTOP_BUTTON)) {
            resId = systemUiResources.getIdentifier(
                        SYSTEMUI_METADATA_NAME + ":drawable/ic_sysbar_expanded_desktop", null, null);
        } else if (clickAction.equals(PaPieConstants.SCREENSHOT_BUTTON)) {
            resId = systemUiResources.getIdentifier(
